...
#pragma comment(lib, "MSIMG32.LIB")	// MSIMG32.LIB 是一个 Windows GDI 库，提供了透明位图等图形操作的函数。

bool is_game_started = false;
bool running = true;

int bullet_num = 3;	//子弹数量

int enemy_generate_speed = 20; 	//敌人生成速度 范围(1-100)
...
public:
	...
	void Draw(int delta)
	{
		int pos_shadow_x = position.x + (FRAME_WIDTH / 2 - SHADOW_WIDTH / 2);	// 阴影居中
...
private:
	IMAGE img_shadow;	//添加阴影
	Animation *anim_left;
	Animation *anim_right;
	POINT position = { 500, 500 };	// 初始玩家位置
...
public:
	...
	bool CheckPlayerCollision(const Player& player)
	{
		...
		bool is_overlap_x = (position.x >= player.GetPosition().x) && (position.x <= player.GetPosition().x + player.FRAME_WIDTH);
		bool is_overlap_y = (position.y >= player.GetPosition().y) && (position.y <= player.GetPosition().y + player.FRAME_HEIGHT);
		return is_overlap_x && is_overlap_y;
	}
...
private:
	...
	const int FRAME_WIDTH = 100;	// 敌人宽度
	const int FRAME_HEIGHT = 100;	// 敌人高度
...
private:
	...
	Animation* anim_left;
	Animation* anim_right;
	POINT position = { 0, 0 };	// 初始敌人位置
	bool facing_left = false;
	bool alive = true;	// 初始敌人生命状态
};

// 生成新的敌人
void TryGenerateEnemy(std::vector<Enemy*>& enemy_list)
{
	int INTERVAL = 101 - (enemy_generate_speed % 101);
...
void UpdateBullets(std::vector<Bullet>& bullet_list, const Player& player)
{
	...
	double radian_interval = 2 * 3.14159 / bullet_list.size();	//子弹间的角度，转化为弧度
...
protected:
	void OnClick()
	{
		...
		mciSendString(_T("play bgm repeat from 0"), NULL, 0, NULL);	// 告诉系统播放bgm，并从开头循环播放
	}
};
...
std::vector<Bullet> updateBulletList()
{
...
int main()
{
	...
	mciSendString(_T("open mus/bgm.mp3 alias bgm"), NULL, 0, NULL);	// 告诉系统将背景音乐添加到程序中,并把这个音乐取名为bgm
	...
	//loadimage(&img_menu, _T("img/OIP.jpg"));
		...
		if (is_game_started)
		{
			...
			for (Enemy* enemy : enemy_list)
			{
				for (const Bullet& bullet : bullet_list)
				{
					...
					{
						mciSendString(_T("play hit from 0"), NULL, 0, NULL);	// 告诉系统播放bgm，并从开头循环播放
						enemy->Hurt();
						score++;
						
						//自己新添加的代码 实现积分每达到10分就增加一颗子弹,敌人移动速度加快
						if (score % 10 == 0)
						{
							bullet_num++;
							if (bullet_num > 20)
								bullet_num = 20;
							bullet_list = updateBulletList();
							enemy_generate_speed += 5;
							if (enemy_generate_speed > 100)
								enemy_generate_speed = 0;
						}
					}
				}
			}
